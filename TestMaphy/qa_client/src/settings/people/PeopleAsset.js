...
class PeopleAsset extends Component {
  constructor(props) {
    super(props)
    this.state = {
      TableData: [],
  ...
  callServerData() {
    const url = Domain + '/hardware?assigned_to=' + this.props.PeopleData.id +
  ...
  render() {
    ...
    return(
      <Redirect  class="small text-white stretched-link" to={{  pathname: './AssetDetails', AssetData: this.state.AssetDetails,mainPage:"PeopleMain"}}>
     </Redirect> )
 /*  //<Link  class="small text-white stretched-link" to={{  pathname: './LicensesMain', licenseprops: {type:"All" }}}>
      // return (<AssetDetails AssetData={this.state.ListAllDataDetails} mainPage="PeopleAsset"/>)
  */
      else
    return (
      <div>
