...
import LicensesUpdate from '../licenses/LicensesUpdate';
...
class PeopleLicenses extends Component {
  constructor(props) {
    super(props)
    this.state = {
      LicensesData: [],
  ...
  callServerData() {
    const url = Domain + '/licenses?assigned_to='+ this.props.PeopleData.id +
  ...
  UpdateLicenses = (licensesid, name) => {
    this.setState({ LicensesId: licensesid, LicensesNametoUpdate: name });
 ...
 {columns
    ...
    if (ShowUpdate) {
      return (
        <LicensesUpdate LicensesId={this.state.LicensesId} LicensesName={this.state.LicensesNametoUpdate} editProcess="Update"/>
      )
    }
    else{
    return (
