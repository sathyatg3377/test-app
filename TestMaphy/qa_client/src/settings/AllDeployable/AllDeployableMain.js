...
import AssetCreate from '../listall/AssetCreate';
import AssetUpdate from '../listall/AssetUpdate';
import AssetDetails from '../listall/AssetDetails';
...
class AllDeployableMain extends Component {
    constructor(props) {
        super(props)
        this.state = {
            value: "en",
            ...
            // },
            // {
            //     dataField: 'expected_checkin.formatted',
            //     text: ' Checkin Date',
            //     sort: false,
            //     hidden: true,
            //     headerStyle:  { 'white-space' : 'nowrap' }
    ...
    callServerData() {
        ...
        const url = Domain + '/hardware?status=rtd' +
    ...
    CheckOutFormatter = (cell, row, rowIndex) => {
        ...
        return (
            <div>
                <button name="btnCheckout" class="btn-sm custombg-checkout custombtn" onClick={() => { if (window.confirm('Are you sure you wish to Checkout these items?')) this.CheckOutAllDeployable(row) }}>{t('button.checkout')}</button>
            </div>
        );
    };
      EditDeleteFormatter = (cell, row, rowIndex) => {
        ...
        return (
            ...
            {(userpermission.superuser === "1") ?
             <>
               <button name="edit" class="btn-sm btn-warning custombtn" data-tooltip="true" title={t('button.edit')}  onClick={() => {  this.UpdateAllDeployable(row) }}><i class="fa fa-edit" aria-hidden="true"></i></button>
               <button name="clone" class="btn-sm btn-info custombtn" data-tooltip="true" title={t('button.clone')}  onClick={() => { this.CloneAllDeployable(row) }}><i class="fa fa-copy" aria-hidden="true"></i></button>
               <button name="delete" class="btn-sm btn-danger custombtn" data-tooltip="true" title={t('button.delete')} onClick={() => { if (window.confirm('Are you sure you wish to delete this item?')) this.deleteAllDeployable(row.id) }}><i class="fa fa-trash" aria-hidden="true"></i></button>
              </>
                 :<button name="print" class="btn-secondary custombtn" data-tooltip="true" title={t('button.print')} onClick={() => {  this.LabelAllDeployable(row) }}><i class="fas fa-print"></i></button>
     ...
     AssetlinkFormatter = (cell, row, rowIndex) => {
        return (
            <div>
                <button class="btn btn-link customlink-btn" name="btnupdate" onClick={() => { this.AssetDetails(row) }}>{row.name}</button>
    ...
    render() {
          const {t} = this.props
        const { ShowCreate, ShowDetails, showCheckOut,  ShowUpdate } = this.state;
        const { AllDeployableData, sizePerPage, page } = this.state;
...
<br />
        ...
        if (ShowCreate) {
            return (
                <AssetCreate mainPage="AllDeployableMain"/>
            )
        }
        else if (ShowUpdate) {
            return (
                <AssetUpdate ListAllDatatoUpdate={this.state.ListAllDatatoUpdate} mainPage="AllDeployableMain" editProcess={this.state.editProcess}/>
            )
        }
     
        else if (showCheckOut) {
            return (
                <CheckOut ListAllDatatoCheckout={this.state.ListAllDatatoUpdate} mainPage="AllDeployableMain"/>
            )
        }
        else if (ShowDetails) {
            return (
                <AssetDetails AssetData={this.state.ListAllDatatoUpdate}  mainPage="AllDeployableMain"/>
            )
        }
else {
            return (
                <div>
                    ...
                    <div >
                        <div class="d-sm-flex align-items-center justify-content-between mb-4">
                            <h1 class="h3 mb-0 text-gray-800 custommain-title" name="deployableassets">{t('AssetsListall.deployableassets')}</h1>
...
export default withTranslation()(AllDeployableMain);