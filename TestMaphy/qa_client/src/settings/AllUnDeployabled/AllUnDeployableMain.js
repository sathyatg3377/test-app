...
class AllUnDeployableMain extends Component {
    constructor(props) {
        super(props)
        this.state = {
            value: "en",
            AllUnDeployableData: [],
            AllUnDeployableId: '',
            AllUnDeployableIdToUpdate: '',
            AllUnDeployableNametoUpdate: '',
            AllUnDeployableDataTotal: '',
        ...
        return (
            ...
            {(userpermission.superuser === "1") ?
             <>
              ...
              :  <button name="print" class="btn-secondary custombtn" data-tooltip="true" title={t('button.print')} onClick={() => { this.LabelAllUnDeployable(row) }}><i class="fas fa-print"></i></button>
          }
            </div>
        );
    };
    ...
    callServerData() {
        ...
        const url = Domain + '/hardware?status=undeployable' +
        ...
        axios.get(url)
            .then((response) => {
                this.setState({ AllUnDeployableData: response.data.rows });
                this.setState({ AllUnDeployableDataTotal: response.data.total });
            });
    }
    ...
    render() {
        ...
        const { AllUnDeployableData, sizePerPage, page } = this.state;
        const { t } = this.props
        ...
        if (ShowCreate) {
            return (
                <AssetCreate mainPage="AllUnDeployableMain"/>
            )
        }
        else if (ShowUpdate) {
            return (
                <AssetUpdate ListAllDatatoUpdate={this.state.ListAllDatatoUpdate} mainPage="AllUnDeployableMain" editProcess={this.state.editProcess}/>
            )
        }

        else if (ShowDetails) {
            return (
                <AssetDetails AssetData={this.state.ListAllDatatoUpdate} mainPage="AllUnDeployableMain"/>
            )
        }
        else {
            return (
                <div>
                    <div >
                        <div class="d-sm-flex align-items-center justify-content-between mb-4">
                            <h1 class="h3 mb-0 text-gray-800 custommain-title">{t('app.undeployed')}</h1>
                        ...
                        <div >
                            <RemoteAll
                                data={AllUnDeployableData}
                                page={page}
                                sizePerPage={sizePerPage}
                                totalSize={this.state.AllUnDeployableDataTotal}
...
export default withTranslation()(AllUnDeployableMain);
