...
import DepreciationCreate from './DepreciationCreate';
import DepreciationUpdate from './DepreciationUpdate';
...
class DepreciationMain extends Component {
  constructor(props) {
    super(props)
    this.state = {
value: "en",
  ...
  callServerData() {
    const url = Domain + '/depreciations?limit=' + this.state.sizePerPage +
    ...
    axios.get(url)
      .then((response) => {
        ...
        this.setState({ DepreciationData: response.data.rows });
        this.setState({ DepreciationDataTotal: response.data.total });
      });
  }
  CreateBtnClick = () => {
...
myColumnToggle = (df,columns) => {
  ...
  EditDeleteFormatter = (cell, row, rowIndex) => {
    const { t } = this.props
      ...
      return (
        <div class="btn-group" role="group" >
  <button name="edit" class="btn-sm btn-warning custombtn" data-tooltip="true" title={t('button.edit')}  onClick={() => {  this.UpdateDepreciation(row) }}><i class="fa fa-edit" aria-hidden="true"></i></button>
  <button name="delete" class="btn-sm btn-danger custombtn" data-tooltip="true" title={t('button.delete')} onClick={() => { if (window.confirm('Are you sure you wish to delete this item?')) this.deleteDepreciation(row.id) }}><i class="fa fa-trash" aria-hidden="true"></i></button>
  ...
  UpdateDepreciation = (row) => {
    console.log("row" + row.name);
    this.setState({ DepreciationDatatoUpdate: row });
...
onLanguageHandle = (event) => {
  ...
  render() {
    ...
    const { DepreciationData, sizePerPage, page } = this.state;
    ...
    if (ShowCreate) {
      ...
      return (
        <DepreciationUpdate DepreciationDatatoUpdate={this.state.DepreciationDatatoUpdate} />
      )
    }
    else {
      return (
        <div class="table-responsive">
          <div>
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
              <h1 class="h3 mb-0 text-gray-800 custommain-title" name="depreciationtitle">{t('depreciation.depreciationtitle')}</h1>
          <div class="row mb-0">
                <div class="col">
...
export default withTranslation()(DepreciationMain);