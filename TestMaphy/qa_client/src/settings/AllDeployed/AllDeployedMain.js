...
import AllDeployedCheckin from './AllDeployedCheckin';
...
class AllDeployedMain extends Component {
    constructor(props) {
        super(props)
        this.state = {
            value: "en",
            AllDeployedData: [],
            AllDeployedDataTotal: '',
...

            {
                ...
                formatter: this.CheckInCheckOutFormatter,
                headerStyle: { 'white-space': 'nowrap' }
            },
    ...
    CheckInCheckOutFormatter = (cell, row, index) => {
        const { t } = this.props

        return (
            <div>
                <button name="btnchekIn" class="btn-sm custombg-checkout custombtn" onClick={() => { this.CheckinAllDeployed(row) }}>{t('button.checkin')}</button>
            </div>
        )
        ...
        return (
            ...
            {(userpermission.superuser === "1") ?
             <>
                ...
                <button name="delete" class="btn-sm btn-danger custombtn" data-tooltip="true" title={t('button.delete')} onClick={() => { if (window.confirm('Are you sure you wish to delete this item?')) this.delete(row.id) }}><i class="fa fa-trash" aria-hidden="true"></i></button>
             </>
    ...
    callServerData() {
        const url = Domain + '/hardware?status=deployed' +
        ...
        axios.get(url)
            .then((response) => {
                this.setState({ AllDeployedData: response.data.rows });
                this.setState({ AllDeployedDataTotal: response.data.total });
            });
    }
    ...
    render() {
        const { t } = this.props
        const { ShowCreate, ShowDetails, showCheckin,  ShowUpdate } = this.state;
        const { AllDeployedData, sizePerPage, page } = this.state;
...
<br />
        ...
        if (ShowCreate) {
            return (
                <AssetCreate mainPage="AllDeployedMain"/>
            )
        }
        else if (ShowUpdate) {
            return (
                <AssetUpdate ListAllDatatoUpdate={this.state.ListAllDatatoUpdate} mainPage="AllDeployedMain" editProcess={this.state.editProcess}/>
            )
        }

        else if (showCheckin) {
            return (
                <AllDeployedCheckin ListAllDatatoCheckIn={this.state.ListAllDatatoUpdate} mainPage="AllDeployedMain"/>
            )
        }
        else if (ShowDetails) {
            return (
                <AssetDetails AssetData={this.state.ListAllDatatoUpdate} mainPage="AllDeployedMain"/>
            )
        }
            ...
            return(
                ...
                <div >
                  <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800 custommain-title" name="allassets">{t('AssetsListall.deployedassets')} </h1>
                 {(userpermission.superuser === "1") ? <button name="create" onClick={this.CreateBtnClick} className="btn btn-sm btn-primary shadow-sm custommain-title">
                  ...
                  <div >
                            ...
                            <RemoteAll
                                data={AllDeployedData}
                                page={page}
                                sizePerPage={sizePerPage}
                                totalSize={this.state.AllDeployedDataTotal}
...
export default withTranslation()(AllDeployedMain);