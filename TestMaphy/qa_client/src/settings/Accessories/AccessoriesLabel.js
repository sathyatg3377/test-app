...
class AccessoriesLabel extends Component {
  constructor(props) {
    super(props)
   this.state = {
    LabelData:'',
  ...
  render() {
    ...
    var selectedAccessory = JSON.parse(localStorage.getItem("AccessoryLabel"));
    const mystyle = {
    ...
    const  AssetQRCode=selectedAccessory.name+'/'+selectedAccessory.category.name+'/'+selectedAccessory.location.name;
    ...
    <div  className="qr_text">
            ...
            <div style={labelstyle} ><strong>{selectedAccessory.company.name}</strong></div>      
            </div>
            <div className="pull-left">
               <div style={labelstyle}><strong>{selectedAccessory.model_number}</strong></div>
            </div>
...
export default AccessoriesLabel;
