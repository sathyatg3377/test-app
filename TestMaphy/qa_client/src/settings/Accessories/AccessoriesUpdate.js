...
class AccessoriesUpdate extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      value: "en",
  ...
  componentDidMount() {
    ...
    this.setState({ AccessoriesName: this.props.AccessoriestoUpdate.name });
    this.setState({ AccessoriesId: this.props.AccessoriestoUpdate.id });
    this.setState({ qty: this.props.AccessoriestoUpdate.qty });
    this.setState({ min_amt: this.props.AccessoriestoUpdate.min_qty });
    this.setState({ model_number: this.props.AccessoriestoUpdate.model_number });
    this.setState({ order_number: this.props.AccessoriestoUpdate.order_number });
    this.setState({ company_id: this.props.AccessoriestoUpdate.company.id });
    this.setState({ location_id: this.props.AccessoriestoUpdate.location.id });
    this.setState({ category_id: this.props.AccessoriestoUpdate.category.id });
    this.setState({ supplier_id: this.props.AccessoriestoUpdate.supplier.id });
    this.setState({ manufacturer_id: this.props.AccessoriestoUpdate.manufacturer.id });

    this.setState({ purchase_date: this.props.AccessoriestoUpdate.purchase_date.date });
    this.setState({ purchase_cost: this.props.AccessoriestoUpdate.purchase_cost });
  ...
  mySubmitHandler = async (event) => {
    ...
    if (this.validateForm()) {
      await axios({
        method: 'put',
        url: Domain + '/accessories/' + this.state.AccessoriesId,
        data: JSON.stringify({
  ...
  render() {
    const { showNotifications, showUpdate, showNotificationsCategory, showNotificationsSupplier, showNotificationsManufacturer, showNotificationsLocation } = this.state;
    const { t } = this.props;
    if (showUpdate) {
      return (
        <div>
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800 custommain-title" name="updateaccessory">{t('accessory.updateaccessory')}</h1>
                          ...
                          <form class="user" onSubmit={this.mySubmitHandler}>
                            ...
                            <div class="form-group row">
                              <div class="col-sm-8 mb-3 mb-sm-0">
                                ...
                                <select ref="suppliers_id" class="form-control" onChange={this.myChangeHandler} id="supplier_id" value={this.state.supplier_id} name="supplier_id">
                                ...
                                <Modal show={this.state.showSupplier} onHide={() => this.handleModalSupplier()}>
                                  ...
                                  <Modal.Body>
                                    ...
                                    <div class="form-group row">
                                      ...
                                      <input type="text" class="form-control "
                                        name="supplier" value={this.state.supplier} id="modal-supplier"
...
export default withTranslation()(AccessoriesUpdate);
