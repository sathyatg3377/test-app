...
class
  LicenseReportMain extends Component {
  constructor(props) {
    super(props)
    this.state = {
      value:"en",
      LicenseReportData: [],
      LicenseReportTotal: 0,
      LicenseReportDataTotal: '',
      offset: 0,
      sortField: 'id',
      sortOrder: 'desc',
      page: 1,
  ...
  callServerData() {
    const url = Domain + '/reports/licenses?limit=' + this.state.sizePerPage +
    ...
    axios.get(url)
      .then((response) => {
        this.setState({ LicenseReportData: response.data.rows });
        this.setState({ LicenseReportDataTotal: response.data.total });
      });
  }
 ...
 myChangeHandler = (event) => {
  ...
  render() {
    ...
    const { LicenseReportData, sizePerPage, page } = this.state;
    const { t } = this.props
        ...
        <div>
          <div >
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
              <h1 class="h3 mb-0 text-gray-800 custommain-title">{t('app.licensereports')}</h1>
         </div>
      <div >
              ...
              <RemoteAll
                ...
                totalSize={this.state.LicenseDataTotal}
                columns={columns}
                CSVExport={CSVExport}
...
export default withTranslation()(LicenseReportMain);
