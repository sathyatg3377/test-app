...
class
  AccessoryReportMain extends Component {
  constructor(props) {
    super(props)
    this.state = {
      value:"en",
      AccessoryReportData: [],
      AccessoryReportTotal: 0,
      AccessoryReportDataTotal: '',
      offset: 0,
      sortField: 'id',
      sortOrder: 'desc',
      page: 1,
  ...
  callServerData() {
    const url = Domain + '/reports/accessories?limit=' + this.state.sizePerPage +
    ...
    axios.get(url)
      .then((response) => {
        this.setState({ AccessoryReportData: response.data.rows });
        this.setState({ AccessoryReportDataTotal: response.data.total });
      });
  }
 ...
 myChangeHandler = (event) => {
  ...
  render() {
    ...
    const { AccessoryReportData, sizePerPage, page } = this.state;
    const { t } = this.props
        ...
        <div>
          <div >
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
              <h1 class="h3 mb-0 text-gray-800 custommain-title">{t('app.accessoryreports')}</h1>
         </div>
      <div >
              ...
              <RemoteAll
                ...
                totalSize={this.state.AccessoryReportDataTotal}
                columns={columns}
                CSVExport={CSVExport}
...
export default withTranslation()(AccessoryReportMain);
