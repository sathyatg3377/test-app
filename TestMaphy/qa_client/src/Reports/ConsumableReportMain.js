...
class ConsumableReportMain extends Component {
  constructor(props) {
    super(props)
    this.state = {
      value:"en",
      ConsumableReportData: [],
      ConsumableReportTotal: 0,
      ConsumableReportDataTotal: '',
      offset: 0,
      sortField: 'id',
      sortOrder: 'desc',
      page: 1,
  ...
  callServerData() {
    const url = Domain + '/reports/consumables?limit=' + this.state.sizePerPage +
    ...
    axios.get(url)
      .then((response) => {
        this.setState({ ConsumableReportData: response.data.rows });
        this.setState({ ConsumableReportDataTotal: response.data.total });
      });
  }
 ...
 myChangeHandler = (event) => {
  ...
  render() {
    ...
    const { ConsumableReportData, sizePerPage, page } = this.state;
    const { t } = this.props
        ...
        <div>
          <div >
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
              <h1 class="h3 mb-0 text-gray-800 custommain-title"> {t('app.consumablereports')}</h1>
         </div>
      <div >
              ...
              <RemoteAll
                ...
                totalSize={this.state.ConsumableReportDataTotal}
                columns={columns}
                CSVExport={CSVExport}
...
export default withTranslation()(ConsumableReportMain);
