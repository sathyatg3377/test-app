...
class
  ComponentsReportMain extends Component {
  constructor(props) {
    super(props)
    this.state = {
      value:"en",
      ComponentsReportData: [],
      ComponentsReportTotal: 0,
      ComponentsReportDataTotal: '',
      offset: 0,
      sortField: 'id',
      sortOrder: 'desc',
      page: 1,
  ...
  callServerData() {
    const url = Domain + '/reports/components?limit=' + this.state.sizePerPage +
    ...
    axios.get(url)
      .then((response) => {
        this.setState({ ComponentsReportData: response.data.rows });
        this.setState({ ComponentsReportDataTotal: response.data.total });
      });
  }
 ...
 myChangeHandler = (event) => {
  ...
  render() {
    ...
    const { ComponentsReportData, sizePerPage, page } = this.state;
    const { t } = this.props
        ...
        <div>
          <div >
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
              <h1 class="h3 mb-0 text-gray-800 custommain-title">{t('app.componentsreports')}</h1>
         </div>
      <div >

              <RemoteAll
                data={ComponentsReportData}
                page={page}
                sizePerPage={sizePerPage}
                totalSize={this.state.ComponentsReportDataTotal}
                columns={columns}
                CSVExport={CSVExport}
...
export default withTranslation()(ComponentsReportMain);
