...
class TicketsByStatus extends Component {
  constructor(props) {
    super(props)
    this.state = {
value:"en",
      ...
      columns: [
        ...
        },
        {
          ...
          sort: false,
          //hidden: false,
          hidden:this.props.ticketStatus === "3"?true:false,
  ...
  DetailsFormatter = (cell, row, rowIndex) => {
    ...
    return (
      <div class="btn-group" role="group" >
        {((row.status !== "Open" &&row.status !== "Close")) ? <button type="button" class="btn-secondary custombtn " onClick={() => { this.UpdateStatus(row) }}> {t('button.status')}</button>
          //: row.status === "Closed" && (userType !== 1) ? <button type="button" class="btn-secondary custombtn " onClick={() => { this.UpdateStatus(row) }}> {t('button.reopen')}</button>
          : <button type="button" class="btn-secondary custombtn " > {t('button.status')}</button>}
      </div>
    );
  };
  render() {
    ...
    if (ShowCreate) {
      ...
      return (<TicketStatusUpdate TicketDetails={this.state.TicketDetails}/>)
    }
    else {
      return (      
        <div>
...
export default withTranslation()(TicketsByStatus);
