...
import TicketIssuesCreate from './TicketIssuesCreate';
import TicketIssuesUpdate from './TicketIssuesUpdate';
...
class TicketIssuesMain extends Component {
  constructor(props) {
    super(props)
    this.state = {
value: "en",
  ...
  callServerData() {
    const url = Domain + '/ticketIssues?limit=' + this.state.sizePerPage +
    ...
    axios.get(url)
      .then((response) => {
 ;
        this.setState({ TicketIssuesData: response.data.rows });
        this.setState({ TicketIssuesDataTotal: response.data.total });
      });
  ...
  deleteTicketIssues = async (id) => {
    const url = Domain + '/ticketIssues/' + id;
  ...
  UpdateTicketIssues = (row) => {
    this.setState({ ticketIssuesDetails: row });
  ...
  render() {
    ...
    const { TicketIssuesData, sizePerPage, page } = this.state;
    ...
    const EditDeleteFormatter = (cell, row, rowIndex) => {
      return (
       <div class="btn-group" role="group" >
          <button name="edit" class=" btn-sm btn-warning custombtn" data-tooltip="true" title={t('button.edit')} onClick={() => { if (window.confirm('Are you sure you wish to update these items?')) this.UpdateTicketIssues(row) }}> <i class="fa fa-edit" aria-hidden="true"></i></button>
      <button name="delete" class=" btn-sm btn-danger custombtn" data-tooltip="true" title={t('button.delete')} onClick={() => { if (window.confirm('Are you sure you wish to delete this item?')) this.deleteTicketIssues(row.id) }}><i class="fa fa-trash" aria-hidden="true"></i></button>
        </div>
      );
    };
    ...
    if (ShowCreate) {
      ...
      return (
        <TicketIssuesUpdate ticketIssuesDetails={this.state.ticketIssuesDetails} />
      )
    }
        ...
        <div>
          <div>
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
              <h1 class="h3 mb-0 text-gray-800 custommain-title" name="issuestitle"> {t('ticketissue.issuestitle')}</h1>
              <div class="row mb-0">
                <div class="col">
          ...
          <div >
                <RemoteAll
                  ...
                  totalSize={this.state.TicketIssuesDataTotal}
                  columns={columns}
                  CSVExport={CSVExport}
...
export default withTranslation()(TicketIssuesMain);
